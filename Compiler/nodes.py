...
class ForNode(Node):
    def Evaluate(self, table):
        valor_final = self.children[1].Evaluate(table)[0]
        symbtable = self.children[0].Evaluate(table)
        valor = symbtable.getter(table)[0]

        while(valor < valor_final):
            self.children[2].Evaluate(table)
            valor += 1
            symbtable.setter(valor, table)
...
class FuncCall(Node):
    def Evaluate(self, table):
        ...
        for i in range(len(self.children)):
            ...
            funcao.children[i].value[1] = retorno_lista[1]
...
class Definition(Node):
    def Evaluate(self, table):
        ...
        if(isinstance(tipo, int) or tipo=="int"):
            table[symbol] = [None, 'int']
        elif(isinstance(tipo, bool) or tipo=="bool"):
...
class Assignment(Node):
    def Evaluate(self, table):
        if(self.children[0].value not in table):
            Definition([self.children[0].value, self.children[1].value]).Evaluate(table)
...
class SymbolTable:
    ...
    def getter(self, table):
        if(self.symbol in table):
            if(table[self.symbol][0] is not None):
