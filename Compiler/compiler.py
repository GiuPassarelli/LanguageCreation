...
class Parser:
    ...
    def parseBlock(isFunction):
        bockTypes = ["EOF", "CLOSE_KEY", "ELSEIF", "ELSE"]
        ...
        while(tokens.actual.type not in bockTypes):
            if(tokens.actual.type == "FUNCTION"):
                ...
                tokens.selectNext()
                if(tokens.actual.type != "OPEN_KEY"):
                    raise Exception("Abra chaves na funcao")
                tokens.selectNext()
                type_ = tokens.actual.type
                if(type_ != "ENTER"):
                ...
                while(tokens.actual.type != "CLOSE_KEY"):
    ...
    def parseCommand(isFunction):
        ...
        if(type_ == "IDENTIFIER"):
            ...
            if(tokens.actual.type != "UNTIL"):
                if(tokens.actual.type != "SEMI_COLLON"):
                    raise Exception("Ponto e virgula")
                tokens.selectNext()
            ...
            forNode = ForNode(None)

            tokens.selectNext()
            forNode.children.append(parser.parseCommand(isFunction))

            forNode.children.append(parser.parseExpression())

            #tokens.selectNext()
            if(tokens.actual.type != "OPEN_KEY"):
                raise Exception("Abra chaves no durante")
            tokens.selectNext()

            if(tokens.actual.type != "ENTER"):
                raise Exception("Pule linha depois do enquanto")

            tokens.selectNext()
            forNode.children.append(parser.parseBlock(isFunction))

            result = forNode

            type_ = tokens.actual.type
            if(type_ != "CLOSE_KEY"):
                raise Exception("Coloque END depois de um enquanto")
            ...
            if(tokens.actual.type != "OPEN_KEY"):
                raise Exception("Abra chaves no enquanto")
            tokens.selectNext()
            ...
            while(type_ == "IF" or type_ == "ELSEIF"):
                ...
                if(tokens.actual.type != "OPEN_KEY"):
                    raise Exception("Abra chves no if")
                tokens.selectNext()
                ...
                if(tokens.actual.type != "CLOSE_KEY"):
                    raise Exception("Feche chves no if")
                tokens.selectNext()

                if(tokens.actual.type != "ENTER"):
                    raise Exception("Pule linhas")
                tokens.selectNext()
